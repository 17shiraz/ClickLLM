...
#for year in dataset:
    # === Config ===
    queries_path = f'/mnt/data/msaliminabi/ClickLLM/data/queries-v2.txt'
    qrels_path = f'/mnt/data/msaliminabi/ClickLLM/data/dbpedia_qrels_abstracts.txt'
    N = None # Set to None to load all samples
    
    # Modify output filename based on mode
    mode_suffix = "abstract" if use_abstract else "title"
    # === Load queries ===
    queries = {}
    ...
    for qid in qid_list:
        ...
        for docid in pos_entities:
            ...
            if match:
                ...
                rate = 0 
            print('binary', model_name, qid, docid, rate)
            with open(f'/mnt/data/msaliminabi/ClickLLM/outputs/llm_qrel/dbpedia/dbpedia_llama4_{mode_suffix}_all.txt', 'a') as output:
